...
# Config
# =========================
DATASET_NAME = "youssefkhalil320/Primock-57-clean"
SPLIT = "train"
NUM_ANNOTATORS = 6  # 57 rows → annotators 0..5 get 10 cases, annotator 5 gets 9

# The dataset has a single column: "conversation"
...
print("NUM_ANNOTATORS:", NUM_ANNOTATORS)
...
# Progress helpers
# =========================
def completed_global_rows_for(annotator_id: int) -> set:
...
def jump_to_case_guarded(annotator_id, jump_to_case):
    ...
    if shard_pos == -1:
        ...
        out[-2] = (
            f"❌ Case {global_row_idx} is not assigned to Annotator {annotator_id}. "
            f"Your cases: {min(shard)}-{max(shard)} (every {NUM_ANNOTATORS}th row)"
        )
        return tuple(out)
...
def imaging_list_to_state(items):
    if not items or not isinstance(items, list):
        return empty_rows(IMAGING_COLS, 1), ["No"]
    rows = []
    contrasts = []
    for it in items:
        if not isinstance(it, dict):
...
def state_to_imaging_items(rows, contrasts):
    ...
    for idx, r in enumerate(rows):
        ...
        item["contrast"] = contrasts[idx] if idx < len(contrasts) else "No"
        if any(v is not None for v in item.values()):
            items.append(item)
    return items if items else None
...
def empty_ui():
    ...
    return (
        ...
        "",             # conversation (replaces dialogue+note+icd10+icd10_desc)
        "",             # debug_box
        ...
        "",             # error_report_content (State)
        gr.update(visible=False),  # error_report_box
...
def load_case(annotator_id, shard_pos):
    ...
    # Primock-57 only has "conversation"
    conversation_val = safe_text(r.get(CONVERSATION_COL, ""))

    debug = (
        f"Annotator={annotator_id} | shard_pos={shard_pos} | global_row={global_i}\n"
        f"Dataset: {DATASET_NAME} | Column: {CONVERSATION_COL}\n"
        f"Total rows: {len(ds)} | Shard size: {len(shard)}\n"
        f"Conversation chars: {len(conversation_val)}"
    )

    done = len(completed_global_rows_for(annotator_id))
    total = len(shard)
    progress_txt = (
        f"Annotator {annotator_id}: {done} done / {total} total "
        f"| Now: {shard_pos + 1}/{total} (global row {global_i})"
    )
    ...
    if isinstance(saved, dict):
        ...
        # BUG FIX from original: key was "Monitoring" on save but "MONITORING" on load
        ...
        # BUG FIX from original: check TREATMENT first, fall back to top-level ext
    ...
    # Contrast radio visibility
    num_imaging_rows = len(imaging_rows)
    radio_updates = []
    for i in range(20):
        if i < num_imaging_rows:
            radio_updates.append(
                gr.update(visible=True, value=imaging_contrasts[i] if i < len(imaging_contrasts) else "No")
            )
        else:
            radio_updates.append(gr.update(visible=False))
    ...
    return (
        ...
        error_content,
        error_report_upd,
    ) + tuple(radio_updates) + (jump_status_msg, status_msg,)
...
def add_imaging_row(imaging_rows, imaging_contrasts):
    imaging_rows = [list(r) for r in (imaging_rows or [])]
    imaging_rows.append(["" for _ in IMAGING_COLS])
    imaging_contrasts = list(imaging_contrasts or [])
...
def save_and_advance_guarded(
    annotator_id, shard_pos,
    labs_rows, imaging_rows, imaging_contrasts,
    ncs_rows, tissue_rows, monitor_rows,
    prevention_rows, conservative_rows, lifestyle_rows,
    meds_state, surg_rows,
    complementary_therapies, external_equipment,
    followup_rows, patient_education, when_to_seek_medical_care,
    referral_rows, report_issue, issue_types, issue_details,
):
    if annotator_id is None or str(annotator_id).strip() == "":
        return empty_ui()
    ...
    # BUG FIX: use "MONITORING" key consistently (was "Monitoring" in original save)
    ...
    record = {
        ...
        "extraction": {
            ...
            # BUG FIX from original: use "MONITORING" (uppercase) to match load key
    ...
    # "All done" signal
    if done == total:
        saved_msg = f"🎉 All {total} cases complete! " + saved_msg
...
def add_new_med(meds_state):
    ...
    new_med["treatment_status"] = "start"
...
def save_med_edit(meds_state, edit_idx, name, treatment_status, frequency, dose, duration,
    ...
    meds_state[edit_idx] = {
        "name": name, "treatment_status": treatment_status, "frequency": frequency,
        "dose": dose, "duration": duration, "discontinuation_criteria": discontinuation,
        "route": route, "dosage_form": dosage_form, "timing_instructions": timing,
        "side_effects_contraindications": side_effects, "drug_class": drug_class,
...
def load_med_for_edit(meds_state, edit_idx):
    ...
    return (
        gr.update(visible=True),
        med.get("name", ""), med.get("treatment_status", "start"),
        med.get("frequency", ""), med.get("dose", ""), med.get("duration", ""),
        med.get("discontinuation_criteria", ""), med.get("route", ""),
        med.get("dosage_form", ""), med.get("timing_instructions", ""),
        med.get("side_effects_contraindications", ""), med.get("drug_class", ""),
        med.get("condition_treated", ""), edit_idx,
    )


def load_med_for_edit_by_number(meds_state, med_num):
    # Guard against None (initial state of gr.Number)
    if med_num is None:
        return (gr.update(visible=False),) + ("",) * 12 + (None,)
    if not meds_state or not isinstance(meds_state, list):
        return (gr.update(visible=False),) + ("",) * 12 + (None,)
    edit_idx = int(med_num) - 1
...
def delete_med(meds_state, med_num):
    ...
    if not meds_state or not isinstance(meds_state, list):
        return meds_state, meds_to_display_df(meds_state), gr.update(visible=False)
    del_idx = int(med_num) - 1
    if del_idx < 0 or del_idx >= len(meds_state):
        return meds_state, meds_to_display_df(meds_state), gr.update(visible=False)
    meds_state.pop(del_idx)
    # BUG FIX from original: also close editor and reset edit_idx
    return meds_state, meds_to_display_df(meds_state), gr.update(visible=False)
...
# UI
# =========================
with gr.Blocks(title="Treatment Plan Extraction - Primock-57", css=CSS) as demo:
    gr.Markdown("# Treatment Plan Extraction — Primock-57")
    gr.Markdown(
        f"⚠️ **Please select an Annotator ID (0–{NUM_ANNOTATORS - 1}) to begin.** "
        f"Dataset: `{DATASET_NAME}` · {len(ds)} conversations · {NUM_ANNOTATORS} annotators"
    )
    ...
    with gr.Row():
        annotator_id = gr.Dropdown(
            ...
            value=None,
            label=f"Annotator ID (0–{NUM_ANNOTATORS - 1})",
        )
    ...
    with gr.Tabs():
        # ------------------------------------------------------------------
        # Conversation tab (replaces Dialogue + Note + ICD10 tabs)
        # ------------------------------------------------------------------
        with gr.Tab("Conversation"):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    # Hidden textbox holds raw text for search logic
                    conversation = gr.Textbox(
                        label="Conversation (raw)",
                        lines=28,
                        interactive=False,
                        visible=False,
                    )
    ...
    with gr.Tabs():
        with gr.Tab("Investigations"):
            ...
            labs_df = gr.Dataframe(
                value=empty_rows(LAB_COLS, 1), headers=LAB_COLS, type="array", interactive=True
            )
            ...
            ncs_df = gr.Dataframe(
                value=empty_rows(NCS_COLS, 1), headers=NCS_COLS, type="array", interactive=True
            )
            ...
            tissue_df = gr.Dataframe(
                value=empty_rows(TISSUE_COLS, 1), headers=TISSUE_COLS, type="array", interactive=True
            )
            ...
            monitor_df = gr.Dataframe(
                value=empty_rows(MONITOR_COLS, 1), headers=MONITOR_COLS, type="array", interactive=True
            )
        ...
        with gr.Tab("Treatment"):
            ...
            prevention_df = gr.Dataframe(
                value=empty_rows(PREVENTION_COLS, 1), headers=PREVENTION_COLS, type="array", interactive=True
            )
            ...
            conservative_df = gr.Dataframe(
                value=empty_rows(CONSERVATIVE_METHOD_COLS, 1),
                headers=CONSERVATIVE_METHOD_COLS,
                type="array",
                interactive=True,
            )
            ...
            lifestyle_df = gr.Dataframe(
                value=empty_rows(LIFESTYLE_COLS, 1), headers=LIFESTYLE_COLS, type="array", interactive=True
            )
            ...
            surg_df = gr.Dataframe(
                value=empty_rows(SURG_COLS, 1), headers=SURG_COLS, type="array", interactive=True
            )
        ...
        with gr.Tab("Follow-up & Other"):
            ...
            followup_df = gr.Dataframe(
                value=empty_rows(FOLLOWUP_COLS, 1), headers=FOLLOWUP_COLS, type="array", interactive=True
            )
            ...
            when_to_seek_medical_care = gr.Textbox(
                ...
                placeholder="When should the patient contact the doctor or go to the hospital?",
                lines=3,
            )
            ...
            referral_df = gr.Dataframe(
                value=empty_rows(REFERRAL_COLS, 1), headers=REFERRAL_COLS, type="array", interactive=True
            )
    ...
    def search_conversation(conv_text, query):
        ...
        return f"<div {base_style}>{highlighted}</div>", f"{count} match{'es' if count != 1 else ''}"

    dialogue_search.change(
        search_conversation,
        inputs=[conversation, dialogue_search],
        outputs=[conversation_display, dialogue_search_status],
    )
    conversation.change(render_dialogue_html, inputs=[conversation], outputs=[conversation_display])

    # ------------------------------------------------------------------
    # DataFrame add/remove/clear wiring
    # ------------------------------------------------------------------
    ...
    btn_labs_clear.click(lambda: df_clear_rows(LAB_COLS), outputs=[labs_df])
    ...
    for idx, radio in enumerate(imaging_contrast_radios):
        def make_handler(row_idx):
            ...
            return handler
        radio.change(make_handler(idx), inputs=[imaging_contrasts_state, radio], outputs=[imaging_contrasts_state])
    ...
    btn_ncs_clear.click(lambda: df_clear_rows(NCS_COLS), outputs=[ncs_df])
    ...
    btn_tissue_clear.click(lambda: df_clear_rows(TISSUE_COLS), outputs=[tissue_df])
    ...
    btn_monitor_clear.click(lambda: df_clear_rows(MONITOR_COLS), outputs=[monitor_df])
    ...
    btn_prev_clear.click(lambda: df_clear_rows(PREVENTION_COLS), outputs=[prevention_df])
    ...
    btn_cons_clear.click(lambda: df_clear_rows(CONSERVATIVE_METHOD_COLS), outputs=[conservative_df])
    ...
    btn_life_clear.click(lambda: df_clear_rows(LIFESTYLE_COLS), outputs=[lifestyle_df])
    ...
    btn_med_add.click(
        ...
        inputs=[meds_state],
        outputs=[
            meds_state, meds_display_df, med_editor, med_select_idx,
            med_name, med_treatment_status, med_frequency, med_dose, med_duration,
            med_discontinuation, med_route, med_dosage_form, med_timing,
            med_side_effects, med_drug_class, med_condition_treated, med_edit_idx,
        ],
    )
    btn_med_edit.click(
        ...
        outputs=[
            med_editor, med_name, med_treatment_status, med_frequency, med_dose,
            med_duration, med_discontinuation, med_route, med_dosage_form, med_timing,
            med_side_effects, med_drug_class, med_condition_treated, med_edit_idx,
        ],
    )
    # BUG FIX: delete now also closes/resets the editor
    btn_med_delete.click(
        delete_med,
        inputs=[meds_state, med_select_idx],
        outputs=[meds_state, meds_display_df, med_editor],
    )
    btn_cancel_med.click(cancel_med_edit, outputs=[med_editor])
    btn_save_med.click(
        save_med_edit,
        inputs=[
            meds_state, med_edit_idx, med_name, med_treatment_status, med_frequency,
            med_dose, med_duration, med_discontinuation, med_route, med_dosage_form,
            med_timing, med_side_effects, med_drug_class, med_condition_treated,
        ],
        outputs=[meds_state, meds_display_df, med_editor],
    )
    ...
    btn_surg_clear.click(lambda: df_clear_rows(SURG_COLS), outputs=[surg_df])
    ...
    btn_fu_clear.click(lambda: df_clear_rows(FOLLOWUP_COLS), outputs=[followup_df])
    ...
    btn_ref_clear.click(lambda: df_clear_rows(REFERRAL_COLS), outputs=[referral_df])

    # ------------------------------------------------------------------
    # Master outputs list — must stay in sync with empty_ui / load_case
    # ------------------------------------------------------------------
    all_outputs = [
        annotator_id,
        shard_pos_state,
        progress,
        conversation,       # single column replaces dialogue/note/icd10/icd10_desc
        debug_box,
        labs_df,
        imaging_df,
        ...
        error_report_box,
    ] + imaging_contrast_radios + [jump_status, status]  # jump_status, status MUST be last two

    demo.load(lambda: empty_ui(), outputs=all_outputs)
    ...
    btn_save.click(
        ...
        inputs=[
            annotator_id, shard_pos_state,
            labs_df, imaging_df, imaging_contrasts_state,
            ncs_df, tissue_df, monitor_df,
            prevention_df, conservative_df, lifestyle_df,
            meds_state, surg_df,
            complementary_therapies, external_equipment,
            followup_df, patient_education, when_to_seek_medical_care,
            referral_df, report_issue, issue_types, issue_details,
        ],
        outputs=all_outputs,
    )
